    ...
    <section className="w-full mx-auto">
      ...
      <ul className="flex justify-between flex-col px-10 mx-auto p-5 rounded-xl w-3/4">
        {posts.map((item) => (
          <li
            key={item.id}
            className="mb-4 break-all p-6 border-4 border-y-black border-x-transparent"
          >
            ...
            <p className="mb-4 mt-2 text-2xl overflow-visible">
              {item.content}
            </p>
            ...
            {item.author ? (
              <Link
                href={`/users/${encodeURIComponent(item.user_id)}`}
                passHref
              >
